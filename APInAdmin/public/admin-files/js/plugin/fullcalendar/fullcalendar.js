	...
	function ResourceWeekView(element, calendar) {
		...
		function render(date, delta) {
			if (delta === 100 || delta === -100) {
				...
				date = new Date();
				var start = new Date(formatDateObj($('#calendar_date_from')));
				var end = new Date(formatDateObj($('#calendar_date_to')));
			}
			...
			t.visStart = visStart;
			t.visEnd = visEnd;
			renderBasic(getResources.length, diffDays, false);
		}
	}
	...
	function ResourceView(element, calendar, viewName) {
		...
		function buildBodyHTML() {
			...
			for (row=0; row<rowCnt; row++) {
				...
				roomType = resources[row]['room_type'];
				html += "<tr class='fc-resourcerow-" + id + "' data-resourceid='" + id + "' data-roomtype='" + roomType + "'>" +
		...
		function colContentLeft(col) {
			return colContentPositions.left(col)+50;
		}


		function colContentRight(col) {
			return colContentPositions.right(col)-50;
		}
	...
	function ResourceEventRenderer() {
		...
		function daySegSetTops(segs, rowTops) { // also triggers eventAfterRender
			...
			for (i=0; i<segCnt; i++) {
				...
				element = seg.element;
				if (element) {
					element[0].style.top = rowTops[seg.row] + 'px';
					event = seg.event;
