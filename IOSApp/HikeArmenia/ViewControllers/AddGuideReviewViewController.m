//
//  AddGuideReviewViewController.m
//  HikeArmenia
//
...
@interface AddGuideReviewViewController ()
...
@implementation AddGuideReviewViewController
...
- (void)viewDidLoad {
	...
	self.navigationItem.title = [NSString stringWithFormat:@"%@ %@",self.guide.firstname,self.guide.lastname];
	...
	attributes = @{NSForegroundColorAttributeName:[UIColor colorWithRed:122.0/255.0 green:160.0/255.0 blue:0.0 alpha:1.0], NSFontAttributeName:[UIFont fontWithName:@"SFUIDisplay-Regular" size:18.0f], NSKernAttributeName: @2.8};
	self.titleLbl.attributedText = [[NSAttributedString alloc] initWithString:@"WRITE A REVIEW" attributes:attributes];
	
	if([[UIScreen mainScreen] bounds].size.height == 480.0)
		self.reviewHeigntConstraint.constant = 100.0;
	else
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    self.screenName = @"Add guide review";
}
...
- (void)saveButtonPressed {
	if([self.reviewTextView.text isEqualToString:@""]) {
        ...
        [alert showCustom:[UIImage imageNamed:@"popupIcon"] color:[UIColor colorWithRed:122.0/255.0 green:160.0/255.0 blue:0/255.0 alpha:1] title:@"Warning" subTitle:@"Please fill the review field"  closeButtonTitle:@"OK" duration:0.0f];
		return;
	}
	...
	[self.guide addReview:self.reviewTextView.text withRating:self.selectedRate callback:^(id result,long long contentLength, NSError *error) {
		if (!error) {
			if([result boolValue])
			{
                SCLAlertView *alert = [[SCLAlertView alloc] initWithNewWindow];
                alert.showAnimationType = SlideInFromTop;
                alert.circleIconHeight = 30.0f;
                [alert showCustom:[UIImage imageNamed:@"popupIcon"] color:[UIColor colorWithRed:122.0/255.0 green:160.0/255.0 blue:0/255.0 alpha:1] title:@"Success" subTitle:@"Review posted"  closeButtonTitle:@"OK" duration:0.0f];
